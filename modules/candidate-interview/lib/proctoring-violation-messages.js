import { proctorAudioKey } from './proctoring-flag-naming.js';

/** Title-case labels for banners, logs, and reports. */
...
/** Maps telemetry / flag audio keys to VIOLATION_LABELS keys. */
export const AUDIO_KEY_TO_VIOLATION = Object.freeze({
...
export const FOCUS_LOSS_WARNING_MESSAGE =
  'Interview window focus lost. Please stay on the interview screen.';

const FIRST_WARNING_AUDIO_TEMPLATES = Object.freeze({
  face_rotation:
...
const FINAL_WARNING_AUDIO_TEMPLATES = Object.freeze({
  webcam_obstruction:
...
export const TERMINATION_BANNER = 'Interview Terminated';
export const TERMINATION_AUDIO =
...
function resolveLexicon(labels = null) {
  const cap = labels?.pageTitle || 'Interview';
  const lower = labels?.labelLower || 'interview';
  return {
    cap,
    lower,
    screen: `${lower} screen`,
    ...
    focusLossWarning:
      labels?.focusLossWarning ||
      `${cap} window focus lost. Please stay on the ${lower} screen.`,
    focusLossSpoken: labels?.focusLossSpoken || `${cap} window focus lost`,
    terminatedTitle: labels?.terminatedTitle || `${cap} Terminated`,
    terminatedAudio:
      labels?.terminatedAudio ||
      labels?.terminatedMessage ||
...
export function buildWarningBanner(flagType, labels = null) {
...
export function buildFinalWarningBanner(flagType, labels = null) {
  const lex = resolveLexicon(labels);
...
export function buildWarningAudio(flagType, labels = null) {
  const lex = resolveLexicon(labels);
  const key = violationKeyFromFlagType(flagType);
  if (isFocusLossViolation(flagType)) return lex.focusLossWarning;
...
export function buildFinalWarningAudio(flagType, labels = null) {
  const lex = resolveLexicon(labels);
  const key = violationKeyFromFlagType(flagType);
  if (isFocusLossViolation(flagType)) {
...
export function buildEscalationMessages(flagType, action, labels = null) {
  const lex = resolveLexicon(labels);
  if (action === 'terminate') {
    return {
      banner: lex.terminatedTitle,
      audio: lex.terminatedAudio,
    };
  }
