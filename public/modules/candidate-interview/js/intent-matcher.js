/**
 * Browser confirmation intent matcher (keep in sync with lib/confirmation-intent.js).
 */
window.ConfirmationIntent = (function () {
  ...
  const CONFIDENCE_PROCEED = 0.65;
  const CONFIDENCE_CLARIFY = 0.45;
  const CONFIDENCE_HIGH = CONFIDENCE_PROCEED;
  const CONFIDENCE_MEDIUM = CONFIDENCE_CLARIFY;
  ...
  /** STT echo of the system confirmation prompt — not a candidate command. */
  function isConfirmationPromptEcho(text) {
  ...
  function patternScore(text, patterns, weightEach) {
    let hits = 0;
    for (const p of patterns) {
      if (p.test(text)) hits += 1;
    }
    return Math.min(1, hits * (weightEach || 0.32));
  }
  ...
  function classifyConfirmation(input) {
    ...
    const repeatCheck = window.QuestionRepeatRequest?.classify?.(raw);
  ...
  function localIntentMatch(alternatives) {
  ...
  function interimIndicatesYes(interimText) {
  ...
  function shouldProceed(result) {
    return (
      ...
      result.confidence >= CONFIDENCE_PROCEED
    );
  }
